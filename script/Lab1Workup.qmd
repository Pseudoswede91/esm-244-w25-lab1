...
Write them below: How many counts of each observation by legal_status? focus on the highest trees?
...
#command shift m gives you the pipe command hotkey
...
ggplot(data = top_5_status, 
       aes(x = legal_status, 
           ...
           y = tree_count)) +
  geom_col(aes(x=fct_reorder(legalstatus))) +
...
Get rid of all the columns that aren't what we want Filter out the plants that are not pine and oak summarize the counts of the oaks and pines

Data wrangle done below: \# \| in string detect str_detect means "or"
